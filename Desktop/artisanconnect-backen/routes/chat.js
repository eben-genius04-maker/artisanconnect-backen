...
router.get('/conversations', requireAuth, async (req, res) => {
  try {
    ...
    let query = supabase.from('conversations').select(
      'id, last_message, last_msg_at, customer:profiles!customer_id(id,full_name,avatar_url), artisan:artisans!artisan_id(id,trade,profiles(full_name,avatar_url))'
...
router.post('/start', requireAuth, async (req, res) => {
  ...
  const { data } = await supabase.from('conversations')
    .insert({ customer_id: req.user.id, artisan_id }).select('id').single();
  res.status(201).json({ conversation_id: data.id });
});
...
router.post('/:id/messages', requireAuth, async (req, res) => {
  ...
  const { data, error } = await supabase.from('messages')
    ...
    .select('id,body,read,created_at,sender:profiles!sender_id(id,full_name,avatar_url)').single();
